...
### LiteLLM 集成修复 (2025-01-11)

**问题**: 通过 LiteLLM 代理的 DeepSeek 认证错误
**问题描述**: 扩展在调用 LiteLLM 代理时错误地将 API 密钥添加到请求体中，导致认证失败。

**根本原因**:

- [`background.js:564`](background.js:564) 将 `payload.api_key = apiKey` 添加到请求体
- LiteLLM 代理尝试使用扩展的代理密钥向 DeepSeek 认证
- 应该只使用 Authorization 头进行代理认证

**应用的修复**:

- 从 `handleLiteLLMRequest` 函数中移除请求负载中的 API 密钥
- API 密钥现在只在 Authorization 头中使用: `Bearer sk-IstlPsekdjLRVkad5ZmhjA`
- LiteLLM 代理正确使用配置的环境变量 (DEEPSEEK_API_KEY, 等)

**关键学习**:

- LiteLLM 代理认证与直接提供商调用不同
- 代理使用环境变量内部处理提供商认证
- 客户端应该只与代理认证，不传递提供商凭据
...
- 模型选择
- 语言设置
- **Max Tokens**: 最大 token 数量（1000-200000，默认 100000）
...
```
用户点击摘要 → HNEnhancer → Summarization → DomUtils (提取评论)
→ ApiClient → Background Script → AI Provider → 返回摘要 → SummaryPanel
```

### 2. 聊天流程

```
用户打开聊天 → ChatModal → DomUtils (收集上下文) → 用户输入消息
→ ApiClient → Background Script → AI Provider → 流式返回 → ChatModal 显示
```

### 3. 导航流程

```
用户按键 → HNEnhancer (键盘处理) → Navigation → DomUtils (查找目标)
→ 更新当前项目 → 滚动到视图
```
...
1. 在 `background.js` 中添加处理器
2. 在 `summarization.js` 中添加方法
3. 在设置页面添加选项
...
- 代理使用环境变量内部处理提供商认证
- 客户端应该只与代理认证，不传递提供商凭据
- 系统知识: 创建了 `/Users/senaca/knowledge_base/litellm_proxy_setup.md` 详细设置信息
...
- 流式和非流式模式都需要适当的错误处理
- 需要修改后台脚本以正确处理流式响应
- LiteLLM 使用 OpenAI 兼容的流式格式，使实现简单直接

**Chrome 扩展消息传递最佳实践**:

- 对于流式或多响应场景，单个 `onMessage` -> `sendResponse` 流程是不够的
- 使用带有特定 `tabId` 的 `chrome.tabs.sendMessage` 比通用 `chrome.runtime.sendMessage` 更可靠
- 使用 Promise 包装整个流式生命周期确保适当的清理时机

### 故障排除指南

**常见问题**:

1. **流式卡住**: 检查消息监听器生命周期和竞争条件
2. **认证失败**: 验证 API 密钥位置 (头部 vs 请求体)
