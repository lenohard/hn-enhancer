...
class ApiClient {
  ...
  async sendBackgroundMessage(type, data = {}) { // Add default value for data
    ...
    try {
      ...
      this.logDebug(
        `Got response from background message '${type}' in ${duration}s. URL: ${
          data?.url || "N/A"
        }`
      );
    } catch (error) {
      ...
      const errorMessage = `Error sending background message '${type}' URL: ${
        data?.url || "N/A"
      }. Duration: ${duration}s. Error: ${error.message}`;
  ...
  async fetchUserInfo(username) {
    try {
      const data = await this.sendBackgroundMessage("FETCH_API_REQUEST", {
  ...
  async fetchHNCommentsFromAPI(itemId) {
    return await this.sendBackgroundMessage("FETCH_API_REQUEST", {
