...
const manifestChrome = JSON.parse(
  fs.readFileSync(new URL("../manifest.chrome.json", import.meta.url))
);
const manifestFirefox = JSON.parse(
  fs.readFileSync(new URL("../manifest.firefox.json", import.meta.url))
);
...
function copySrcDirectory(targetDir) {
  ...
  srcFiles.forEach((file) => {
    ...
    if (fs.existsSync(sourcePath)) {
      ...
      console.warn(
        `Warning: Source file ${sourcePath} does not exist, skipping.`
      );
    }
  });
}
