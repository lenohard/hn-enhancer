...
class ChatModal {
  ...
  open(commentElement, postId) {
    ...
    this.enhancer.logInfo(
      `Opening chat for post ${postId}, comment ID: ${commentId}`
    );
  ...
  openForPost(postId) {
    ...
    if (this.isVisible && this.isPostChat) {
      this.enhancer.logDebug(
        "Post chat modal already open, toggling visibility."
      );
      this.hide();
      return;
    }
  ...
  _addCommentLinkHandlers(container) {
    const commentLinks = container.querySelectorAll(
  ...
  async _gatherContextAndInitiateChat(contextType = this.currentContextType) {
    ...
    if (!postId || !commentId) {
      console.error(
        "Missing postId or commentId in _gatherContextAndInitiateChat"
      );
      this._displayMessage(
        "Error: Cannot initiate chat due to missing identifiers.",
        "system"
      );
      return;
    }
    ...
    this.enhancer.logDebug(
      `Initiating chat for post ${postId}, comment ${commentId}, context: ${contextType}`
    );
    ...
    try {
      ...
      if (loadedHistory && loadedHistory.length > 0) {
        ...
        this.conversationHistory.forEach((message) => {
          if (message.role === "system") {
            ...
            const contextInfo = this._extractContextInfoFromSystemMessage(
              message.content
            );
            // 显示指令部分和评论统计信息
        ...
        if (!aiProvider) {
          this.enhancer.logInfo(
            "Chat: AI provider not configured (history loaded)."
          );
          ...
          this.enhancer.logInfo(
            `Chat: Using AI Provider: ${aiProvider}, Model: ${
              model || "default"
            } (history loaded)`
          );
      ...
      this.enhancer.logDebug(
        `No history found for ${postId}/${commentId}/${contextType}. Gathering context...`
      );
      // this._displayMessage(`Gathering ${contextType} context...`, "system"); // Show gathering status

      let contextArray = [];
      const targetCommentId = this.enhancer.domUtils.getCommentId(
        this.targetCommentElement
      );
      const targetAuthor = this.enhancer.domUtils.getCommentAuthor(
        this.targetCommentElement
      );
      const targetText = this.enhancer.domUtils.getCommentText(
        this.targetCommentElement
      );
      ...
      switch (contextType) {
        ...
        case "descendants":
          ...
          const targetReplies = this.enhancer.domUtils.getDirectChildComments(
            this.targetCommentElement
          ).length;
          const targetDownvotes =
            this.enhancer.domUtils.getDownvoteCount(
          ...
          const descendants = this.enhancer.domUtils.getDescendantComments(
            this.targetCommentElement
          );
        ...
        case "children":
          ...
          const targetRepliesForChildren =
            this.enhancer.domUtils.getDirectChildComments(
              this.targetCommentElement
            ).length;
          const targetDownvotesForChildren =
            this.enhancer.domUtils.getDownvoteCount(
              this.targetCommentElement.querySelector(".commtext")
            ) || 0;
        ...
        default:
          ...
          this._displayMessage(
            `Error: Invalid context type selected: ${contextType}.`,
            "system"
          );
          return;
      }
...
`;
      ...
      this.enhancer.logDebug(
        "Initialized conversation history with system prompt and context."
      );
      ...
      const totalChars = contextArray.reduce(
        (sum, c) => sum + (c.text?.length || 0),
        0
      );
      // 从系统消息中提取指令部分
      const promptText = initialSystemMessage.content
        .substring(0, initialSystemMessage.content.indexOf("评论上下文:"))
        .trim();
      this._displayMessage(
  ...
  async _sendMessageToAI(conversationHistory) {
    ...
    if (aiProvider === "chrome-ai") {
      ...
      try {
        ...
        if (!lastUserMessage) {
          throw new Error(
            "No user message found in history to send to Chrome AI."
          );
        }

        this.enhancer.logDebug(
          "Sending last user message to Chrome AI:",
          lastUserMessage
        );
    ...
    try {
      ...
      this.enhancer.logDebug(
        `Added ${aiProvider} response to history:`,
        this.conversationHistory
      );
  ...
  async _switchContext(newContextType) {
    if (newContextType === this.currentContextType) {
      this.enhancer.logDebug(
        `Context type already set to ${newContextType}, no change needed.`
      );
  ...
  _extractContextInfoFromSystemMessage(systemMessage) {
    ...
    try {
      ...
      console.error(
        "Error extracting context info from system message:",
        error
      );
    }
  ...
  async _gatherPostContextAndInitiateChat(contextType = "descendants") {
    ...
    this.enhancer.logDebug(
      `Initiating post chat for post ${this.currentPostId}, context: ${contextType}`
    );
    ...
    try {
      ...
      // --- No History Found - Gather Context ---
      this.enhancer.logDebug(
        `No history found for post ${this.currentPostId}/${contextType}. Gathering context...`
      );
      ...
      if (contextType === "descendants") {
        ...
        allComments.forEach((commentElement) => {
          const commentId = this.enhancer.domUtils.getCommentId(commentElement);
          const author =
            this.enhancer.domUtils.getCommentAuthor(commentElement);
          ...
          if (commentId && author !== null) {
            ...
            const downvotes =
              this.enhancer.domUtils.getDownvoteCount(
                commentElement.querySelector(".commtext")
              ) || 0;
            const directChildren =
              this.enhancer.domUtils.getDirectChildComments(commentElement);
            const replyCount = directChildren.length;

            // Calculate score
            const score = this.enhancer.domUtils.calculateCommentScore(
              commentIndex,
              allComments.length,
              downvotes
            );
...
`;
      ...
      this.conversationHistory.push(initialSystemMessage);
      this.enhancer.logDebug(
        "Initialized conversation history with system prompt and post context."
      );
      ...
      // 从系统消息中提取指令部分
      const promptText = initialSystemMessage.content
        .substring(0, initialSystemMessage.content.indexOf("帖子上下文:"))
        .trim();
      this._displayMessage(
        `${promptText}\n\n[已加载 ${contextType} 上下文: ${contextArray.length} 条评论 (${totalChars} 字符)。请输入您的消息。]`,
        "system"
  ...
  _extractCommentPathsFromSystemMessage(systemMessage) {
    try {
      ...
      this.enhancer.logDebug(
        `从系统消息中提取了 ${this.commentPathToIdMap.size} 个评论路径到ID的映射`
      );
    } catch (error) {
  ...
  async _initializeChromeAISessionIfNeeded() {
    if (
      this.enhancer.isChomeAiAvailable !== HNEnhancer.CHROME_AI_AVAILABLE.YES
    ) {
      ...
      try {
        ...
        this.enhancer.isChomeAiAvailable = availability;
        this.enhancer.logDebug(
          `Chrome AI availability re-check: ${availability}`
        );
      } catch (err) {
    ...
    if (
      ...
      this.enhancer.logDebug("Chrome AI selected but not available/ready.");
      this._displayMessage(
        "Chrome Built-in AI is selected but not available or ready.",
        "system"
      );
      // Optionally suggest changing settings
    }
